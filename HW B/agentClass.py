...
class TQAgent:
    ...
    def fn_init(self,gameboard):
        ...
        positions = gameboard.N_col # We have N_col number of horizontal positions for each piece. 
        rotations = 4 # We four possible positions for each piece. 
        
        self.currentState = np.zeros(gameboard.N_row*gameboard.N_col + len(gameboard.tiles)) # Possible states are defined by gameboard size and the next piece. 
        
        self.actions = []
        for i in range(0, positions):
            for j in range(0, rotations):
                self.actions.append((i,j)) # Each action is a tuple of position and rotation. 

        self.actions = np.array(self.actions)

        self.Qtable = np.zeros((2**(self.gameboard.N_row * self.gameboard.N_col + len(gameboard.tiles)), len(self.actions))) # 2** as we for each position have either 1 or 0 (or -1).
        self.rewards = np.zeros(self.episode_count) # Maximal number of moves. 
    ...
    def fn_read_state(self):
        ...
        currentBoard = np.ndarray.flatten(self.gameboard.board) # Previously stored states as an array. 
        binaryCurrentBoard = np.where(currentBoard == -1, 0, currentBoard) # Want a binary represenation. 
        
        nextTile = np.zeros(len(self.gameboard.tiles)) # Add the state.
        nextTile[self.gameboard.cur_tile_type] = 1

        binaryCurrentState = np.concatenate((binaryCurrentBoard, nextTile))
        binaryCurrentStateString = ''.join(str(int(x)) for x in binaryCurrentState) # int() does not take lists. 
        self.currentStateIndex = int(binaryCurrentStateString, 2)
    ...
    def fn_select_action(self):
        ...
        if r < self.epsilon:
            while(not hasMoved):
                self.currentActionIndex = np.random.randint(0, len(self.actions)) # Choose action randomly. 
                move = self.gameboard.fn_move(self.actions[self.currentActionIndex][0], self.actions[self.currentActionIndex][1]) # Try to make the move. 

                if move == 0:
                    hasMoved = True # Success. 
                else:
                    self.Qtable[self.currentStateIndex, self.currentActionIndex] = -np.inf # Illegal move, do not choose it again. 
        else:
            while(not hasMoved):
                # Look at the Qtable at the current board state and choose the maximal value. 
                self.currentActionIndex = np.where(self.Qtable[self.currentStateIndex, :] == np.max(self.Qtable[self.currentStateIndex, :]))[0]

                if len(self.currentActionIndex) > 1:
                    self.currentActionIndex = self.currentActionIndex[np.random.randint(0, len(self.currentActionIndex))] # If multiple maxima, choose one randomly. 
                else:
                    self.currentActionIndex = self.currentActionIndex[0]
    ...
    def fn_reinforce(self, old_state, reward):
        ...
        old_action = self.currentActionIndex
        self.Qtable[old_state, old_action] = self.Qtable[old_state, old_action] + self.alpha*(reward + np.max(self.Qtable[self.currentStateIndex, :]) - self.Qtable[old_state, old_action])
    ...
    def fn_turn(self):
        if self.gameboard.gameover:
            ...
            if self.episode%100==0:
                print('episode '+str(self.episode)+'/'+str(self.episode_count)+' (reward: ',str(np.sum(self.rewards[range(self.episode-100,self.episode)])),')')
            if self.episode%1000==0:
                ...
                if self.episode in saveEpisodes:
                    ...
                    #np.savetxt("strategy_file" + str(self.episode) + ".csv", self.Qtable, delimiter=",") # Too big!
                    np.savetxt("Rewards_c.csv", self.rewards)
            ...
            old_state = self.currentStateIndex
            ...
            self.rewards[self.episode] = self.rewards[self.episode] + reward
